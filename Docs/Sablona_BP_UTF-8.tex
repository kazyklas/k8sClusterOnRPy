...
\subsection{Škálovatelnost a management klastru}


\section{Ansible}

\subsection{Příprava nasazení}


\section{Docker}

\subsection{Co je kontejner?}

\subsection{Můj Docker image}
...
Jsou to takové funkce f: X ® Y, pro něž je snadné z jakékoli hodnoty x Î X vypočítat y = f(x), ale pro náhodně vybraný obraz y Î f(X) nelze v relevatním čase najít její vzor x Î X tak, aby y = f(x).Přitom víme, že takový vzor existuje nebo jich existuje dokonce velmi mnoho. To kolik jich existuje se odvíjí jakou hašovací funkci použijeme. K těmto funkcím se váží pojmy, bez kterých bychom se v této práci neobešli a proto si je zadefinujeme.

\subsection{Narozeninový paradox}

\subsection{Známé hašovací funkce}


\section{Formáty ukládání v operačních systémech}
